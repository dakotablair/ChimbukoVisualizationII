...
class ProvDB:
    def __init__(
        ...
        print(
            dedent(f"""
                ProvDB initialization commencing with parameters
                pdb_path={pdb_path}
                pdb_sharded_num={int(pdb_sharded_num)}
                pdb_addr={pdb_addr}
                pdb_addr_path={pdb_addr_path}
                pdb_ninstance={int(pdb_ninstance)}
            """)
        )
        # A map of shard index to a tuple of (server instance, provider index)
        self.pdb_shard_map = (
            []
        )
        ...
        if (pdb_addr == "" and pdb_addr_path == ""):
            ...
            if pdb_path and int(pdb_sharded_num) > 0:
                for i in range(int(pdb_sharded_num)):
                    ...
                    self.pdb_admin[0].attach_database(
                        self.pdb_address[0],
                        provider,
                        pdb_name,
                        "unqlite",
        ...
        # setup output (in which case pbd_addr is ignored)
            ...
            if self.pdb_ninstance == 1 and pdb_addr != "":
                ...
                self.pdb_engine = [
                    Engine(pdb_addr.split(":")[0], pymargo.client)
                ]
                self.pdb_address = [pdb_addr]
                self.pdb_client = [SonataClient(self.pdb_engine[0])]
                print(
                    "Attached to single remote server instance on address %s"
                    % self.pdb_address[0]
                )
                for i in range(int(pdb_sharded_num)):
                    # provider index for server 0 shard i is always i+1  (0 is
                    # reserved for the global database)
                    self.pdb_shard_map.append(
                        (0, i + 1)
                    )

            else:
                # pdb_addr is ignored. pdb_addr_path must be set to the output
                # path of the database where addresses and shard->instance maps
                # are located
                # Path contains provider.map which maps
                # shard -> (instance, provider)
                # and provider.address.${i} which contains the ip:port for
                # server instance i
                if pdb_addr_path == "":
                    raise Exception(dedent("""
                        pdb_addr_path must be set if #instances>1 or
                        #instances==1 and pdb_addr is not supplied
                    """))
                ...
                for i in range(int(pdb_sharded_num)):
                    ...
                    if len(l) != 3:
                        raise Exception(
                            dedent(f"""
                                Line "{line}" in provider.map does not have
                                expected format
                            """)
                        )
                    if int(l[0]) != i:
                ...
                for i in range(self.pdb_ninstance):
                    f = open(f"{pdb_addr_path}/provider.address.{i}", "r")
                    self.pdb_address.append(f.readline())
                    f.close()

                # We want the engine to be initialized with the openfabrics
                # provider type used to communicate with the clients (eg
                # ofi+tcp;ofi_rxm)
                # In principle different providers could be used by different
                # server instances so we allow for multiple engines
                engine_ofiprov = []
                ...
                for a in self.pdb_address:
                    ...
                    for e in range(len(engine_ofiprov)):
                        if engine_ofiprov[e] == ofiprov:
                            eng_idx = e
                            break
                    if eng_idx == None:  # noqa: E711
                        eng_idx = len(engine_ofiprov)
                        print(
                            "Creating engine %d with provider %s"
                            % (eng_idx, ofiprov)
                        )
                ...
                for i in range(self.pdb_ninstance):
                    ...
                    self.pdb_client.append(
                        SonataClient(self.pdb_engine[eng_idx])
                    )
                    print(
                        "Attached to remote server instance %d on address %s"
                        % (i, self.pdb_address[i])
                    )
        ...
        for i in range(pdb_sharded_num):
            ...
            database = self.pdb_client[inst].open(
                self.pdb_address[inst], provider, pdb_name
            )
            print(
                dedent(f"""
                    Attached to database shard {i} residing on server instance
                    {inst} and provider {provider}.
                """)
            )
        ...
        print(f"=-=-=Initiated ProvDB instance {self.pdb_address}=-=-=")

    def __del__(self):
        if self.pdb_databases:
            for database in self.pdb_databases:
                del database
                database = None  # noqa: F841
        ...
        if self.pdb_names:
            for i in range(len(self.pdb_names)):
                ...
                if hasattr(self, "pdb_admin"):  # Only in offline mode
                    (inst, provider) = self.pdb_shard_map[i]
                    self.pdb_admin[inst].detach_database(
                        self.pdb_address[inst], provider, name
                    )
                del name
                name = None
            self.pdb_names = []
