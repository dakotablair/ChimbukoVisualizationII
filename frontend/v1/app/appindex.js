...
class ChimbukoApp extends React.Component {
  ...
  handleHistoryRequest = (rank) => {
    ...
    if (watched_ranks.indexOf(rank) >= 0) return;
  ...
  handleHistoryRemove = (rank) => {
    const watched_ranks = [...this.props.watched_ranks];
    if (watched_ranks.length == 0) return;
  ...
  handleTreeRequest = (key) => {
    if (this.props.set_value) this.props.set_value("node_key", key);
  };
  ...
  render() {
    ...
    const getSelectedName = () => {
      ...
      if (parseInt(fid) == -1) {
        ...
        if (parseInt(rank) == -1) return `a${app}f${fid}s${step1}e${step2}`;
        else return `a${app}r${rank}f${fid}s${step1}e${step2}`;
      }
    };
...
export default withStyles(styles)(
  connect(mapStateToProps, mapDispatchToProps)(ChimbukoApp)
);
