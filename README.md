...
![Overview](./data/images/latest.png)
...
* `Dynamic Top MPI Ranks and Functions` (top left): Streaming data from the anomaly detection module is aggregated in the back end into two separate statistics about the application or workflow's overall performance in the current timeframe. The first statistics shows the top N ranks that have the most anomalous functions. The other statistics show the top N anomalous functions that happen at most ranks. These two series of bar graphs present a dynamic ranking dashboard and highlight the ranks and functions that are most problematic and users should pay attention to. Users can select how many (N) ranks and functions to keep track of. Interactive operations are also supported for users to click any top rank/function and see more details about what the anomalies are for that time interval.
* `Dynamic Radar chart` (top middle left): This panel shows the top individual functions that have the highest severity. The radar chart shows all the relevant properties of these functions all at once (e.g., app, rank, fid, severity, score, and count). The legends are clickable allowing users to check and track desired functions in detail.
* `Dynamic Ranks and Functions Distributions` (top middle right): This panel shows the corresponding global distribution of the top N rank/function as line plots. It presents as a supplementary view of not just the top problematic locations of the application or workflow, but also the overall anomaly distribution for both ranks and functions. Users can click any data point of the line plot to find what the corresponding anomalies are.
* `Provenance Database Query` (top right): This panel serves as the interface for users to enter filtering conditions to query the provenance database. As mentioned above, users have the option to click the two dynamic panels or directly input the function execution information in this panel. All the queried results will be visualized in the next panels as a platform for the drill-down study by the users.
...
To connect to the Provenance database of Chimbuko where all the actual function execution data are stored, [`py-sonata`](https://xgitlab.cels.anl.gov/sds/py-sonata) needs to be installed as well. It requires both [spack](https://spack.io/) and [sonata](https://xgitlab.cels.anl.gov/sds/sonata). 
